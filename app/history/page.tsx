  ...
  return (
    <PhoneFrame>
      <div className="flex items-center gap-2 mb-1 flex-wrap">
        <div className="text-[19px] md:text-[26px] font-bold font-display">{t("hist.title")}</div>
        <RoleBadge role={role} />
      </div>
      <div className="text-[12.5px] text-slate mb-4">
        {t("hist.sub", { name: name || (role === "runner" ? t("role.runner") : t("role.community")) })}
      </div>
      ...
      {!loaded ? (
        ...
        <div className="text-center bg-white border border-dashed border-line rounded-card px-5 py-10">
          ...
          <div className="font-display font-bold text-[16px] mb-1">{t("hist.noHistory")}</div>
          <div className="text-[12px] text-slate leading-relaxed mb-4">
            {t("hist.noHistoryBody")}
          </div>
          <Link href={role === "runner" ? "/dashboard" : "/browse"} className="w-full block">
            <button className="w-full bg-orange text-white rounded-[10px] px-4 py-2.5 text-[12.5px] font-semibold">
              {role === "runner" ? t("hist.goDashboard") : t("hist.findRunner")}
            </button>
          </Link>
        </div>
      ) : (
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <div className="bg-white border border-line rounded-[12px] p-3 text-center">
            ...
            <div className="text-[9.5px] text-slate uppercase tracking-wide mt-0.5">{t("hist.completed")}</div>
          </div>
          <div className="bg-white border border-line rounded-[12px] p-3 text-center">
            ...
            <div className="text-[9.5px] text-slate uppercase tracking-wide mt-0.5">
              {role === "runner" ? t("hist.earned") : t("hist.spent")}
            </div>
          </div>
          <div className="bg-white border border-line rounded-[12px] p-3 text-center">
            <div className="font-mono font-bold text-[18px] text-slate">{cancelledCount}</div>
            <div className="text-[9.5px] text-slate uppercase tracking-wide mt-0.5">{t("hist.cancelled")}</div>
          </div>
        </div>
        ...
        <div className="flex bg-paper2 rounded-[10px] p-[3px] mb-4 w-fit">
          {(
            [
              { value: "all", label: t("hist.tabAll") },
              { value: "done", label: t("hist.tabCompleted") },
              { value: "cancelled", label: t("hist.tabCancelled") },
              { value: "expired", label: t("hist.tabExpired") },
            ] as const
          ).map((tabItem) => (
            <button
              key={tabItem.value}
              ...
              className={`px-3.5 py-2 text-xs font-semibold rounded-lg transition-colors ${
                tab === tabItem.value ? "bg-white text-ink shadow-sm" : "text-slate hover:text-ink"
              }`}
            >
              {tabItem.label}
            </button>
        ...
        <div className="flex flex-wrap items-end gap-2 mb-4">
          <div>
            <label className="text-[10.5px] font-semibold text-slate block mb-1">{t("hist.from")}</label>
            <input
              type="date"
              value={fromDate}
          ...
          <div>
            <label className="text-[10.5px] font-semibold text-slate block mb-1">{t("hist.to")}</label>
            <input
              type="date"
              value={toDate}
        ...
        {filtered.length === 0 ? (
          <div className="text-center bg-white border border-dashed border-line rounded-card px-5 py-10">
            ...
            <div className="font-display font-bold text-[16px] mb-1">{t("hist.nothingInRange")}</div>
            <div className="text-[12px] text-slate leading-relaxed">
              {t("hist.nothingInRangeBody")}
            </div>
          </div>
        ) : (
            ...
            {note.total && (
              <div className="flex items-center justify-between rounded-[10px] bg-[#E4F3EC] border border-[#C8E6DA] px-3 py-2 mt-2">
                <span className="text-[11.5px] font-semibold text-teal">
                  {role === "runner" ? t("hist.communityPays") : t("hist.youPaid")}
                </span>
